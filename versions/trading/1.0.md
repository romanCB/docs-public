...
Different programming languages represent primitives differently. The Candlebets specification supports by name only the primitive types supported by the [JSON-Schema]. However, in order to allow fine tuning a primitive definition, an additional [`format`](#dataTypeFormat) field MAY accompany the [`type`](#dataTypeType) primitive to give more information about the type used. If the [`format`](#dataTypeFormat) field is used, the respective client MUST conform to the elaborate type.
...
---|:---|---:|---
code | `string` | {1-20} | **Required.** error code eg. TRADER_NOT_FOUND, INTERNAL_ERROR, OK
...
See status object [`status`](#Status)
