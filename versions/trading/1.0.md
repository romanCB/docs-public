...
Queuing mechanism uses machine to machine authentication e.g. TLS secure channel (but playerId must be passed to the domain application in format of JWT). Require **reactive approach** to the solution and the following integration patterns to be employed:  
...
[`order.cancel`](#orderCancel) | Async | Cancel pending game/bet
...
Each http call for 1.0 protocol version should follows the pattern `scheme:[//host[:port]][/]v1.0[/]path`.  

Each queue name should be prefixed with protocol version it is supporting (one protocol version per queue/exchange). The pattern is as follow: `v1.0.orders.open`. Both major and minor part of the protocol version number is important.  
